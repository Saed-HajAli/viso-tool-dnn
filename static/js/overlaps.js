  ...
  var problow , probhigh , acls , pcls , res ;
    ...
    if(acls != "all") 
    {
        tableData = data.filter(function(d){
          ...
          if(status == 3) if(d[model.target] != acls)
            return false;

          if(status == 3) if(pcls != "all" && d[model.predicted] != pcls)
            return false;

          if(status == 2) if(d["P-"+acls] < problow || d["P-"+acls] > probhigh)
            return false;

          if(status == 1 || status == 2) if(d["L-"+acls].toLowerCase() != res)
            return false;
