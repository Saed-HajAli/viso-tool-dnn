function confMatrix(model , settings , parent){
	...
	mind = model.matrixBound.mind
	maxd = model.matrixBound.maxd
	max = model.matrixBound.max
	min = model.matrixBound.min

	this.matData = this.newData = model.confMatrixData;
    ...
    this.prepareData = function(mat){
    	...
    	for(i=0 , len = (classes).length;i<len;i++){
			for(j=0;j<len;j++){
				...
				if(classes[i] != classes[j] && c < min)
					min = c;
				if(classes[i] != classes[j] && c > max)
					max = c;
				if(classes[i] == classes[j] && c < mind)
					mind = c;
				if(classes[i] == classes[j] && c > maxd)
					maxd = c;
			}
		}
    }
	...
	this.overlap = function(nowdata){
		pane = settings.confPane;
		nowmat = model.confMatrixData ; // getConfusionMatrix(nowdata, model.classNames, model.target, model.predicted);
